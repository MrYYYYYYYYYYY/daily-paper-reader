...
tags: ["query:lpt"]
score: 6.0
evidence: 解决检索可靠性以缓解大模型问答中的幻觉问题
tldr: 现有GraphRAG将自动构建的图视为独立知识源，但图是不完整投影，导致检索和生成不可靠。PAGE-RAG将图视为语义骨架，不取代原始文档，采用投影感知的自适应检索路由，根据查询需求动态选择检索行为，并实施严格知识边界控制，确保生成基于证据。实验表明，该方法在长文档问答中达到竞争性答案质量，同时提升检索效率和知识可靠性，强调了投影感知图建模、自适应检索和知识边界控制对可信GraphRAG的重要性。
source: arxiv
...
motivation: 自动构建的图是源文档的不完整投影，直接作为知识源不可靠，需要投影感知建模。
method: 将图视为语义骨架，引入任务自适应检索路由和严格知识边界控制，动态选择检索行为并约束生成。
result: 在长文档问答中答案质量竞争，检索效率和知识可靠性提升。
conclusion: 投影感知图建模与自适应检索是关键，确保GraphRAG可信。
---

## 摘要
GraphRAG通过引入超越传统检索的结构化表示来改进长文档问答。然而，自动构建的图本质上是源文档的不完整投影，将其视为独立的知识源可能导致不可靠的检索和生成。我们提出PAGE-RAG，一种投影感知的自适应图检索框架，用于可靠的长文档问答。PAGE-RAG将图结构视为组织和导航文档知识的语义骨架，而非替代原始知识源。基于这一视角，PAGE-RAG引入了任务自适应检索路由策略，根据查询需求动态选择合适的检索行为。此外，PAGE-RAG还包含了严格的知识边界控制，确保生成的响应始终基于可用证据，并在超出可及知识范围时放弃支持。实验表明，PAGE-RAG在提高检索效率和知识可靠性的同时，达到了有竞争力的答案质量，突显了投影感知图建模、自适应检索和明确的知识边界控制对于可信GraphRAG系统的重要性。源代码已公开在https://github.com/CXY0112/PAGE-RAG。
